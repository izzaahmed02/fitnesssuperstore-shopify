...
## Addendum 3 — the logs route is closed, and the funnel shows no 30 May impact

`docs/qa/evidence/supaeasy-logs-empty-2026-09-20.webp`

### SupaEasy logs cannot answer this

The Logs tab reads **"No logs found"** with the retention stated on the screen:

> We will retrieve up to 100 function logs from the last 10 minutes.

**Ten minutes.** So the app's logs can never speak to 30 May, or to any date
beyond the last few minutes. The function is also disabled, so it is not
executing and produces no logs at all.

This is the third retention wall in this workstream:

| Source | Reach |
|---|---|
| Shopify function run log | 7 days (Tim already flagged 16 Aug - 9 Sep as unrecoverable) |
| Abandoned checkouts | oldest retrievable is 8 June 2026 |
| SupaEasy function logs | **last 10 minutes** |

The per-invocation detail for 30 May onward is therefore **not recoverable from
any self-serve source we hold.** Re-enabling the rule to generate logs would not
recover history and would put a blocking rule back in front of live customers,
so it is not an option.

The banner is generated by **Shopify** from its own checkout-rule error
telemetry, not by SupaEasy. So the affected-checkout count and error detail have
to come from **Shopify Support**, quoting the banner on Settings > Checkout and
the 30 May 2026 start date. SupaEasy support is the route for the separate
question of when the rule was enabled and disabled.

### The funnel shows no impact at the 30 May boundary

Independent of any log, the checkout funnel survives retention. Store-wide, from
`FROM sessions SHOW sessions_that_reached_checkout, sessions_that_completed_checkout`:

Weekly, checkout completion rate:

| week | reached | completed | rate |
|---|---|---|---|
| 2026-05-11 | 296 | 72 | 24.3% |
| 2026-05-18 | 331 | 85 | 25.7% |
| **2026-05-25** | 259 | 80 | **30.9%** |
| **2026-06-01** | 209 | 68 | **32.5%** |
| 2026-06-08 | 213 | 55 | 25.8% |
| 2026-06-15 | 194 | 71 | 36.6% |

Daily across the boundary:

| day | reached | completed | rate |
|---|---|---|---|
| 2026-05-28 | 44 | 13 | 29.5% |
| 2026-05-29 | 28 | 10 | 35.7% |
| **2026-05-30** | 24 | 7 | **29.2%** |
| 2026-05-31 | 25 | 9 | 36.0% |
| 2026-06-01 | 43 | 15 | 34.9% |

There is **no drop on or after 30 May**. The two weeks straddling the date are in
fact the strongest in the window.

Read this as bounding the impact, not as clearing it. At roughly 250 checkouts a
week, a handful of blocked carts would not move the aggregate. What it does rule
out is a broad checkout outage running since 30 May. It is consistent with a rare
edge case, which is what a cart reaching a $0 subtotal would be.

### Where this leaves the question

- The banner is real and Shopify-generated.
- The only rule configured to block is SupaEasy's `$0 subtotal validator`, and it
  is now fully off.
- The instruction-limit defect fails silently and undercharges, so it does not
  explain "unable to submit orders", and its errors start 10 September regardless.
- No self-serve source can produce the 30 May onward detail.
- The store-level funnel shows no degradation at the boundary, so whatever
  happened was small in volume.

Recommendation: ask Shopify Support for the affected-checkout count behind the
banner rather than spending more time on app-side logs, and treat the impact as
low until that number says otherwise. Nothing here bears on the Post Avis Phase 2
candidate.
