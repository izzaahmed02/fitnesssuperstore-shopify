...
| 8 | Shopify sync back — **HELD by default** | Do **not** make an unapproved live-theme admin edit to test back-sync. This test is held unless separately approved in writing; when approved, perform it as a **no-visible-impact edit on an unpublished (non-live) theme** connected for the test, confirm the resulting `Update from Shopify …` commit lands on the expected branch, then revert the edit. Never edit the live MAIN theme to satisfy this test. Forward sync (test 7) already confirms the integration is live; back-sync branch confirmation can otherwise be verified from existing history. |
