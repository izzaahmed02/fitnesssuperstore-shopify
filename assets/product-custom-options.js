  ...
  customElements.define(
    ...
    class ProductCustomizationOptions extends HTMLElement {
      ...
      async replaceItem() {
        ...
        const selectedQuantity = Number(this.quantityInput?.value || this.dataset.lineItemQuantity || 1);
        ...
        try {
          ...
          const removeResponse = await fetch(changeUrl, removeConfig);
          const removeResult = await removeResponse.json();
          if (!removeResponse.ok) throw new Error('Failed to remove previous cart item');

          const finalResult = removeResult?.sections ? removeResult : updateResult;
          if (window.location.href.includes('/cart')) {
            this.getSectionsToRender().forEach((section) => {
              const elementToReplace = document.querySelector(section.selector) || document.getElementById(section.id);

              elementToReplace.innerHTML = this.getSectionInnerHTML(finalResult.sections[section.section], section.selector);
            });
          } else {
            this.cartDrawer.renderContents(finalResult);
          }
