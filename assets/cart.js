class CartRemoveButton extends HTMLElement {
	...
	async removeItem() {
		...
		if (cartItems) {
			...
			if (itemKey) {
        cartItems.updateOptions(itemKey, 0, itemIndex);
      } else {
...
class CartItems extends HTMLElement {
  ...
  async updateOptions(itemKey, inputValue, index) {
        ...
        if (!currentItem.properties?._parent_product_id && currentItem.properties?._group_id) {
          ...
          if (relatedItems.length > 0) {
            for (const item of relatedItems) {
              updates[item.key] = inputValue;
            }
          }

          updates[itemKey] = inputValue;
          await this.updateItemsQuantity(updates);
          this.disableLoading(index);
          return;

        } else {
          this.updateQuantity(index, inputValue);
        }
      } else {
        this.updateQuantity(index, inputValue);
      }
	...
	validateQuantity(event) {
      ...
      if (itemKey) {
        this.updateOptions(itemKey, inputValue, index);
      } else {
        this.updateQuantity(
          index,
          inputValue,
...
window.updateCartTotals = async function () {
    ...
    cartData.items.forEach((item) => {
      if (!item.properties?._parent_product_id) {
        itemsQuantity = itemsQuantity + item.quantity;
      }
    });

		const cartHeadingSpan = document.querySelector('cart-items .title--primary span');
    ...
    if (cartHeadingSpan) {
      cartHeadingSpan.setAttribute('data-count', itemsQuantity);
      cartHeadingSpan.innerHTML = itemsQuantity;
    }
    if (drawerHeadingSpan) {
      drawerHeadingSpan.setAttribute('data-count', itemsQuantity);
      drawerHeadingSpan.innerHTML = itemsQuantity;
    }
    if (cartCount) {
      cartCount.innerHTML = itemsQuantity;
    }
...
async function clearCart() {
	...
	try {
		// Disable the clear cart buttons
		...
		// Get the cart data after clearing
		const cartData = await response.json();

		// Re-render the cart and drawer
		...
		// Re-enable the clear cart buttons
...
function updateInput() {
	let updated = false;
	const shippingCalc = document.querySelector('#cart__wrapper .docapp-cart-with-shipping-summary .docapp-shipping-calculator--rates');
	const activeInput = shippingCalc && shippingCalc.querySelector('input[checked]');
	if (activeInput && activeInput.parentElement.innerHTML.toLowerCase().includes('in-store pickup')) {
		const nextInput = shippingCalc.querySelectorAll('input');
		nextInput.forEach(item => {
			if (!item.parentElement.innerHTML.toLowerCase().includes('in-store pickup')) {
				shippinCalcObserver.disconnect();
...
let ratesObserver;
const shippinCalcObserver = new MutationObserver((items) => {
	items.forEach(item => {
		if (item.target.classList.value.includes('docapp-shipping-calculator--rates')) {
			ratesObserver = new MutationObserver((rateItems) => {
				if (!document.querySelector('.docapp-shipping-calculator--loader').classList.value.includes('docapp-active')) {
					updateInput();
				}
			});

			ratesObserver.observe(document.querySelector('.docapp-shipping-calculator--loader'), {
				childList: true,
				subtree: true,
				attributes: true,
...
shippinCalcObserver.observe(document.querySelector('.cart__footer'), {
	childList: true,
	subtree: true,
});

function addListenerEstimate() {
	if (document.querySelector('.docapp-cart-with-shipping-summary--summary').getAttribute('shippingListener') === "1") {
		return
	}
	document.querySelector('.docapp-shipping-calculator--button').addEventListener('click', function(e) {
		const shippingInput = document.querySelectorAll('#cart__wrapper .docapp-cart-with-shipping-summary .docapp-shipping-calculator--rates input');
		shippingInput.forEach(item => {
			item.addEventListener('change', (e)=> {
				if (e.target.parentElement.innerHTML.toLowerCase().includes('in-store pickup')) {
					shippingInput.forEach(newInput => {
						if (!newInput.parentElement.innerHTML.toLowerCase().includes('in-store pickup')) {
							newInput.parentElement.click();
						}
					});
				}
			});
	...
	document.querySelector('.docapp-cart-with-shipping-summary--summary').setAttribute('shippingListener', "1");
}
