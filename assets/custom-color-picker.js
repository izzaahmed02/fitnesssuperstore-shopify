...
document.addEventListener('DOMContentLoaded', function () {
  const colorUtils = {
    ...
    buildGradient(colorString) {
      ...
      if (parts.length === 1) {
        const hex = this.getHexFromName(parts[0]);
        return `linear-gradient(to right, ${hex}, ${hex})`;
      }
      if (parts.length === 2) {
        const leftColor = this.getHexFromName(parts[0]);
        const rightColor = this.getHexFromName(parts[1]);
      ...
      const stops = hexes.map((hex, i) => `${hex} ${Math.round(i * step)}%`).join(', ');
  ...
  class CustomColorManager {
    ...
    updateColorOption() {
      const apoColors = document.querySelectorAll(`.ap-options__swatch-container input[field-name="${this.groupColorName} Color"]`);
      ...
      if (window.ProductConfigurator) {
        ...
        if (productOptions) {
          ...
          if (configurator && configurator.selectedOptions) {
            ...
            const mainProductIndex = configurator.selectedOptions.findIndex((opt) => opt.id.toString() === mainProductId.toString());
            if (mainProductIndex >= 0) {
              configurator.selectedOptions[mainProductIndex].properties[`${this.groupColorName} Color`] = `Custom Color: ${color}`;
  ...
  class ColorGroupManager {
    ...
    initializeUIElements() {
      ...
      this.apoColors = document.querySelectorAll(`.ap-options__swatch-container input[field-name="${this.groupColorName} Color"]`);
    }
    ...
    handleCloseSelectedInfo() {
      ...
      const apoOptionColorSelected = Array.from(document.querySelectorAll('.ap-options__swatch-container .option_selected')).find((div) => div.textContent.trim().includes(this.currentColorName));
      ...
      const customColorAvis = document.querySelector(`.custom-color-${this.toLowerCaseFirstLetter(this.groupColorName)}-avis input`);
      if (customColorAvis) {
  ...
  if (groupColorContainers.length) {
    const colorGroupManagers = Array.from(groupColorContainers).map((container, index) => new ColorGroupManager(container, index));
...
window.reinitializeColorPicker = function () {
  ...
  if (groupColorContainers.length) {
    console.log('Found color group containers:', groupColorContainers.length);

    // Handle visibility and UI adjustments
    const visibleContainers = Array.from(groupColorContainers).filter((group) => {
      return group.offsetParent !== null;
    });
