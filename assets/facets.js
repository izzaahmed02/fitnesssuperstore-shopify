...
onSubmitHandler(event) {
  ...
  forms.forEach((form) => {
    ...
    if (priceMinTextInput && priceMaxTextInput) {
      ...
      // Update text inputs
      priceMinTextInput.value = minValue > 0 ? minValue.toFixed(2) : '';
      priceMaxTextInput.value = maxValue < textMax ? maxValue.toFixed(2) : '';
...
class PriceRangeSlider extends HTMLElement {
  ...
  init() {
    ...
    // Get max values from attributes
    const rangeMax = Number(this.maxSlider.max); // e.g., 17399
    const textMax = Number(this.textInputs[1]?.getAttribute('data-max').replace(/,/g, '')); // e.g., 17399.00

    // Initialize values from text inputs, removing commas
  ...
  syncTextInputs() {
    ...
    // Update sliders (no conversion, as range inputs are in whole units)
    this.minSlider.value = Math.round(minValue);
    this.maxSlider.value = Math.round(maxValue);
