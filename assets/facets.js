...
customElements.define('facet-remove', FacetRemove);
          ...
          // CORS Test Script for Shopify Storefront API
async function testStorefrontAPICORS() {
  console.log('Starting CORS test for Storefront API...');

  // Use variables from Liquid code
  ...
  // Simple GraphQL query to test connectivity (fetch shop name)
  const testQuery = `
    query {
      shop {
        name
  ...
  try {
    ...
    const response = await fetch(apiUrl, {
      ...
      headers: {
        ...
        'b12e91e3ef7ffa7bfa432bedbcd0ab85': storefrontToken,
      },
    ...
    // Log success
    console.log('CORS Test Success: Storefront API request completed successfully.');
    console.log('Shop Name:', data?.shop?.name || 'Not available');
  } catch (error) {
    // Log detailed error
    console.error('CORS Test Failed: Error making Storefront API request.');
    console.error('Error Details:', error.message);
    console.error('API URL:', apiUrl);
    console.error('Token Used:', storefrontToken ? 'Present' : 'Missing');
    
    // Check for CORS-specific errors
    if (error.message.includes('Access-Control-Allow-Origin') || error.message.includes('CORS')) {
      console.error('CORS Issue Detected: The server blocked the request due to CORS policy.');
      console.error('Check the X-Shopify-Storefront-Access-Token and ensure it is valid for', shopifyDomain);
    } else if (error.message.includes('401') || error.message.includes('Unauthorized')) {
      console.error('Authentication Issue: Invalid or unauthorized Storefront API token.');
      console.error('Verify the token in Shopify Admin > Apps > Your App > Storefront API.');
    } else {
      console.error('Other Issue: May be network-related or server configuration.');
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('Initiating CORS test on page load...');
  testStorefrontAPICORS();
});
