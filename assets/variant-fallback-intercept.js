 ...
 *    Shopify's normal flow then updates ?variant=<id> in place.
 *
 *  - Combined-listing parent (data-is-combined-listing="true"): the
 *    parent's aggregated picker lets customers click combinations that
 *    don't have a backing variant; without a fallback the page reloads
 *    with the default variant and the click appears to revert. Intercept
 *    every option click, resolve the intended combination against the
 *    map, and navigate to the matching (or closest) variant's URL —
 *    which carries the ?variant=<id> the destination needs to render
 *    the right options as selected.
 */
(function () {
  ...
  function handleCombinedClick(event, productInfo, variantsMap) {
    // Combined-listing parent: every option click needs to land on a variant
    // URL. If the customer picks an impossible combination, route to the
    // closest matching variant so the destination page renders correctly
    // (instead of falling back to the parent's default variant).
    const label = event.target.closest('[data-variant-options] label[for]');
    let input;
    if (label) {
      const forId = label.getAttribute('for');
