				...
				if (existingOptions.length > 0) {
					...
					existingOptions.forEach(option => {
						...
						if (matchingInput) {
							...
							console.log(`Variant is not mutched, ID: ${variantId}`);
						}
					});
				}

				const colorGroup = popup.querySelectorAll('.group-color-container');
				colorGroup.forEach(item => {
					const colorName = parentItem.properties[`${item.getAttribute("data-group-color-name")} Color`];
					if (!item.querySelector(`.swatch[data-color-name="${colorName}"]`)) {
						const customColorInput = item.querySelector('.custom-color-input');
						customColorInput.querySelector('input').value = colorName.replace('Custom Color', '').replace(':', '').trim();
						customColorInput.querySelector('button').dispatchEvent(new Event('click', { bubbles: true }));
					} else {
						const selectedColor = item.querySelector(`.color-options .swatch[data-color-name="${colorName.replace('Color', '').replace(':', '').trim()}"]`);
						if (selectedColor) {
							selectedColor.dispatchEvent(new Event('click', { bubbles: true }));
						}
					}
				});
			}
...
function collectOptionsData(popup, parentItem) {
	...
	const updatedParentProperties = Object.fromEntries(
		Object.entries(parentItem.properties).filter(([key]) => key.indexOf('_') >= 0)
	  );
	...
	const updateParentColorProperty = (colorType, colorValue, customColor) => {
		const prefTmp = customColor ? 'Custom Color: ' : '';
		updatedParentProperties[colorType + ' Color'] = prefTmp + colorValue;
	};

	const basicOptions = popup.querySelectorAll('.product-options__accordion[data-type="Basic"], .product-options__accordion[data-type="Assembly & Room of Choice Installation Needed?"]');
		...
		if (optionInput) {
			if (optionInput.length === 1) {
				...
				if (optionPrice === 0) {
					...
					updatedParentProperties[optionName] = optionValue;
				}
			} else {
	...
	customColorGroups.forEach(group => {
		...
		if (paintContainer) {
			const colorSelectedSwatch = paintContainer.querySelector('.swatch.color-selected:not(.swatch--custom-trigger)');
			if (colorSelectedSwatch) {
				updateParentColorProperty('Paint', colorSelectedSwatch.getAttribute('data-title'), false);
			} else {
				...
				if (customColorPaintHidden && customColorPaintHidden.value) {
					...
					if (variantId) {
						...
						updateParentColorProperty('Paint', customColorPaintHidden.value, true);
						selectedOptions.unshift({
							...
							properties: {
								...
								_option_title: `Paint Color - Custom ${customColorPaintHidden.value}`,
		...
		if (vinylContainer) {
			const colorSelectedSwatch = vinylContainer.querySelector('.swatch.color-selected:not(.swatch--custom-trigger)');
			if (colorSelectedSwatch) {
				updateParentColorProperty('Vinyl', colorSelectedSwatch.getAttribute('data-title'), false);
			} else {
				const customColorVinylHidden = group.querySelector('#custom-color-vinyl-hidden', true);
					...
					if (variantId) {
						...
						selectedOptions.unshift({
							...
							properties: {
								...
								// _unique_key: timestamp + '-' + randomString + '-' + variantId,
								_group_id: newGroupId,
								_option_title: `Vinyl Color - Custom ${customColorPaintHidden.value}`,
	...
	radioInputs.forEach(input => {
		...
		const optionPrice = parseFloat(wrapper.querySelector('.product-options__swatch-price').getAttribute('avis-price'));

		if (optionPrice > 0) {
			selectedOptions.unshift({
				id: variantId,
				quantity: quantity * parentItem.quantity,
				properties: {
					_parent_product_id: parentItem.variant_id,
					_option_title: fieldName,
					// _unique_key: timestamp + '-' + randomString + '-' + variantId,
					_group_id: newGroupId
				}
			});
