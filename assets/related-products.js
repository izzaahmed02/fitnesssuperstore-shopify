document.addEventListener('DOMContentLoaded', function () {
  ...
  // Function to init a slider for lazy-loaded image areas
  function initSlider(element) {
    if (!$(element).hasClass('slick-initialized')) {
      ...
      // Load lazy images inside the slider
  ...
  // Observe .image-wrap to init sliders when in view
  ...
  // Init Slick on recommendations after section is AJAX-loaded
  document.addEventListener('shopify:section:load', function (event) {
    const container = event.target.querySelector('.related-products .featured_products_container');
  ...
  // Fallback: init on DOM ready in case recommendations are rendered server-side
  const $staticContainer = $('.related-products .featured_products_container');
  if ($staticContainer.length && !$staticContainer.hasClass('slick-initialized')) {
    $staticContainer.slick({
      dots: false,
      arrows: true,
      infinite: false,
      autoplay: false,
